    ...
    <section className="mb-20">
      ...
      <div className="flex items-center gap-4 mb-8">
        <div className="flex items-center gap-2">
          <span className="block w-1 h-8 rounded-full bg-gold" />
          <h2
      ...
      <div className="relative grid md:grid-cols-5 rounded-2xl overflow-hidden shadow-2xl border border-midnight/10 bg-midnight group">
        ...
        */}
        <div className="md:col-span-2 p-7 md:p-10 flex flex-col justify-between bg-gradient-to-br from-[#12123a] to-[#0a0a22]">
          {/* Top */}
          <div>
            <span className="inline-block text-xs uppercase tracking-widest text-gold font-semibold mb-5">
              Latest Release
            </span>
            <h3
              className="text-xl md:text-2xl font-black text-white leading-snug mb-5"
              style={{ fontFamily: "'Playfair Display', serif" }}
            >
              {title}
            </h3>
            ...
            {tags && tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 bg-white/10 hover:bg-gold/20 border border-white/15 hover:border-gold/40 text-white/70 hover:text-gold text-xs font-medium rounded-full transition-colors cursor-default select-none"
                  >
                    #{tag}
                  </span>
                ))}
