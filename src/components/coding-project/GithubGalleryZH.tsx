  ...
  return (
    <div className="flex items-center gap-3">
      <a href={url} target="_blank" rel="noopener noreferrer"
        className="text-xs font-mono truncate w-44 hover:underline flex-shrink-0 text-[#191970] dark:text-[#d4af37]"
        title={name}>{name}</a>
...
function ActivityPanelZH({ items, color, emptyText }: { items: RepoContribution[]; color: string; emptyText: string }) {
  if (!items.length) return <p className="text-sm text-gray-400 dark:text-gray-500 py-4 text-center">{emptyText}</p>;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500">共 {items.length} 个仓库</span>
      </div>
      {items.map((item, i) => (
        <RepoBar key={`${item.repository.url}-${i}`}
          name={item.repository.name} url={item.repository.url}
          count={item.contributions.totalCount} max={max} color={color} />
      ))}
    </div>
  );
}
...
export function GithubGalleryZH() {
  ...
  const loadGithubData = async () => {
    ...
    try {
      ...
      setUser(userData); setRepositories(reposData); setPinnedRepos(pinnedData);
      setContributions(contribData); setLanguages(languagesData); setDetailedActivity(activityData);
    } catch {
    ...
    return (
      <div className="mb-10 pb-10 border-b border-gray-200 dark:border-gray-700">
        <div className="flex gap-6 mb-8 items-start">
          ...
          <div>
            <h2 className="text-2xl font-bold mb-0.5 text-[#191970] dark:text-[#d4af37]">{user.name || user.login}</h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-2">@{user.login}</p>
            {user.bio && <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed max-w-lg">{user.bio}</p>}
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 sm:gap-6">
          {[
            ...
            <div key={label}
              className="text-center rounded-xl py-4 px-2 border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-[#1e1e1e]">
  ...
  const renderPinnedRepos = () => {
    if (isLoading) return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-5 text-[#191970] dark:text-[#d4af37]">精选仓库</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1,2,3].map(i => <div key={i} className="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-gray-700 rounded-xl p-5 animate-pulse h-40" />)}
        </div>
      </section>
    );
              ...
              <div key={`${repo.url}-${idx}`}
                ...
                <h3 className="font-semibold text-base mb-1">
                  <a href={repo.url} target="_blank" rel="noopener noreferrer"
                    className="hover:underline text-[#191970] dark:text-[#d4af37]">{repo.name}</a>
                </h3>
                <p className="text-gray-500 dark:text-gray-400 text-sm mb-4 flex-1 line-clamp-2">
                  {repo.description || '无描述'}
                </p>
                <div className="flex flex-wrap gap-3 items-center text-xs text-gray-400 dark:text-gray-500 mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setViewerRepo(repo.name)}
                    className="flex-1 flex items-center justify-center gap-1.5 text-sm font-medium py-2 px-3
                  ...
                  <a href={repo.url} target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center px-3 py-2 rounded-lg
                      ...
                      hover:border-gray-400 dark:hover:border-gray-400 text-sm transition-colors">
                    GitHub
                  </a>
                </div>
              </div>
  ...
  const renderContributions = () => {
    if (isLoading) return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-5 text-[#191970] dark:text-[#d4af37]">贡献活动</h2>
    ...
    return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-1 text-[#191970] dark:text-[#d4af37]">贡献活动</h2>
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-5">
          过去一年内 <strong className="text-[#191970] dark:text-[#d4af37]">{contributions.totalContributions}</strong> 次贡献
        </p>
        <div className="overflow-x-auto">
          <div className="inline-flex gap-1 p-4 bg-white dark:bg-[#1e1e1e] rounded-xl border border-gray-200 dark:border-gray-700">
            {weeks.map((week, wi) => (
  ...
  const renderLanguages = () => (
    <section className="mb-12">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xl font-bold text-[#191970] dark:text-[#d4af37]">编程语言</h2>
        {!isLoading && languages.length > 0 && (
          <span className="text-sm text-gray-400 dark:text-gray-500">{languages.length} 种语言</span>
        )}
      </div>
  ...
  const renderRepositories = () => {
    if (isLoading) return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-5 text-[#191970] dark:text-[#d4af37]">公开仓库</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {repositories.map((repo, idx) => (
            <div key={`${repo.url}-${idx}`}
              ...
              <p className="text-gray-500 dark:text-gray-400 text-xs mb-4 flex-1 line-clamp-2">
                {repo.description || '无描述'}
              </p>
              ...
              <div className="h-0.5 bg-green-100 dark:bg-green-900/30 rounded mb-3">
                <div className="h-full bg-green-400 dark:bg-green-600 rounded w-full" />
              </div>
              <button onClick={() => setViewerRepo(repo.name)}
  ...
  const renderDetailedActivity = () => {
    ...
    if (isLoading) return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-5 text-[#191970] dark:text-[#d4af37]">贡献详情</h2>
        <div className="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-gray-700 rounded-xl p-6 animate-pulse space-y-4">
          <div className="flex gap-3">{[1,2,3].map(i => <div key={i} className="h-8 w-24 bg-gray-200 dark:bg-gray-700 rounded-lg" />)}</div>
          {[1,2,3,4].map(i => (
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-3 text-sm font-medium transition-all border-b-2 -mb-px
                    ${isActive
                      ? 'bg-white dark:bg-[#1e1e1e] border-current'
                  ...
                  {count > 0 && (
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold
                      ${isActive ? 'text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}
    ...
    <div className="w-full">
      {error && (
        <div className="mb-8 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800
          rounded-xl text-red-700 dark:text-red-400 text-sm flex items-center justify-between">
          <span>{error}</span>
