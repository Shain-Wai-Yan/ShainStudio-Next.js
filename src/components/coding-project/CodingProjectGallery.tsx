...
interface CodingProjectGalleryProps {
  images: string[];
  title: string;
  language: 'en' | 'zh';
}

export default function CodingProjectGallery({ images, title, language }: CodingProjectGalleryProps) {
  // ── Visible grid state ──────────────────────────────────────────────────────
  ...
  const hasNext = isOpen && selectedIndex! < images.length - 1;
  ...
  }, [hasPrev, setSelectedIndex, setIsZoomed]);
  ...
  }, [hasNext, setSelectedIndex, setIsZoomed]);
