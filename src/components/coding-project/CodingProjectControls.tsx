...
import type { FilterOptions } from '@/lib/strapi/coding-projects';

interface CodingProjectControlsProps {
  search: string;
  filters: { category: string; tools: string; tag: string; type: string };
  filterOptions: FilterOptions;
  onSearchChange: (value: string) => void;
  onFilterChange: (key: keyof CodingProjectControlsProps['filters'], value: string) => void;
  onRefresh: () => void;
...
export function CodingProjectControls({
  search,
  filters,
  filterOptions,
  onSearchChange,
  ...
  return (
    <section
      className="sticky top-[70px] z-40 bg-white/95 dark:bg-[#121212]/95 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 shadow-sm transition-colors duration-200"
    ...
    >
      ...
      <div className="hidden md:flex max-w-7xl mx-auto px-4 sm:px-6 py-3 items-center gap-2 flex-wrap">
        ...
        {/* Clear */}
        {hasAnyActive && (
          <button onClick={handleClearAll} className="h-10 px-3 text-xs font-medium text-red-500 dark:text-red-400 border border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors focus:outline-none rounded">
            Clear all
          </button>
        )}
      ...
      <div className="md:hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2.5">
          ...
          <button
            ...
            className={`
              relative flex-shrink-0 flex items-center gap-1.5 h-10 px-3 rounded
              ...
              ${filtersOpen || activeFilterCount > 0
                ? 'bg-[#191970] dark:bg-[#d4af37] text-white dark:text-[#121212] border-[#191970] dark:border-[#d4af37]'
          ...
          >
            ...
            Filters
            {activeFilterCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-[#d4af37] text-[#191970] text-[9px] font-black flex items-center justify-center">
                {activeFilterCount}
              </span>
            )}
          </button>
  ...
  return (
    <div className="relative w-full md:w-[240px] lg:w-[320px]">
      ...
      <input
        ...
        className="
          w-full pl-9 pr-8 h-10 rounded
          border border-gray-200 dark:border-gray-700
          bg-white dark:bg-[#1e1e1e]
          text-gray-900 dark:text-[#e0e0e0] text-sm
          placeholder:text-gray-400 dark:placeholder:text-gray-500
          focus:outline-none focus:border-[#191970] dark:focus:border-[#d4af37]
          focus:ring-1 focus:ring-[#191970]/20 dark:focus:ring-[#d4af37]/20
  ...
  return (
    <select
      ...
      className={`
        ...
        focus:outline-none focus:border-[#191970] dark:focus:border-[#d4af37]
        focus:ring-1 focus:ring-[#191970]/10 dark:focus:ring-[#d4af37]/20
        transition-all duration-150
        ${fullWidth ? 'w-full' : 'min-w-[120px] max-w-[160px]'}
        ${value ? 'text-[#191970] dark:text-[#d4af37] border-[#191970]/40 dark:border-[#d4af37]/40' : ''}
      `}
      style={{
...
function RefreshButton({ onClick, isRefreshing, label }: { onClick: () => void; isRefreshing: boolean; label: string }) {
  return (
    <button
      ...
      title={label}
      className="
        flex-shrink-0 flex items-center justify-center w-10 h-10 rounded
        border border-gray-200 dark:border-gray-700
        bg-white dark:bg-[#1e1e1e]
...
function Chip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 text-[10px] font-medium bg-gray-100 dark:bg-[#2a2a2a] text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded max-w-[160px]">
