...
// Safely parse percentage strings like "54.2" → 54.2
...
export function ProgrammingLanguages({ languages, isLoading }: ProgrammingLanguagesProps) {
  ...
  // Build / rebuild chart when lib or data ready
  useEffect(() => {
    ...
    const border = isDark ? '#1a1a1a' : '#ffffff';
  ...
  if (isLoading) {
    return (
      <section className="mb-12">
        <h2 className="text-xl font-bold mb-5" style={{ color: '#191970' }}>Programming Languages</h2>
        <div className="bg-white border border-gray-200 rounded-xl p-6 animate-pulse">
          ...
          <div className="h-1.5 bg-gray-200 rounded-full mt-5"/>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mb-12">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xl font-bold" style={{ color: '#191970' }}>Programming Languages</h2>
        ...
        <div className="flex items-center gap-6 sm:gap-10 flex-wrap">
          ...
          <div className="relative flex-shrink-0" style={{ width: 160, height: 160 }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-xl font-bold" style={{ color: '#191970' }}>
                {parseFloat(activeLang.percentage).toFixed(1)}%
              </span>
              <span className="text-xs text-gray-400 mt-0.5 max-w-[80px] text-center leading-tight truncate">
                {activeLang.name}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-[180px] space-y-1">
            {languages.map((lang, i) => (
              ...
              >
                ...
                />
                <span className="flex-1 text-xs text-gray-700 truncate">{lang.name}</span>
                <span
