...
export default function VideoModal({
  ...
  const labels = locale === 'zh'
    ? { share: '分享', shareTitle: '分享或复制链接', youtube: '在 YouTube 观看', close: '关闭', views: '次播放', previous: '上一个', next: '下一个', navigate: '切换', closeHint: '关闭' }
    : { share: 'Share', shareTitle: 'Share or copy link', youtube: 'Watch on YouTube', close: 'Close', views: 'views', previous: 'Previous', next: 'Next', navigate: 'navigate', closeHint: 'close' };
  // ── Keyboard navigation ─────────────────────────────────────────────────
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mb-5">
              <span className="flex items-center gap-1.5 text-xs text-white/50">
                ...
                <span className="text-white/70">{video.viewCount} {labels.views}</span>
              </span>
              {video.duration && (
                <>
        ...
        {(onPrev || onNext) && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-white/10 bg-black/30 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-white/25 text-xs">
              ...
              <span className="ml-0.5">{labels.closeHint}</span>
            </div>
